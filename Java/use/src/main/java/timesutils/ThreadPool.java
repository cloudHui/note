...
public class ThreadPool {
	...
	public ThreadPool(String prefix, int size) {
		...
		this.size = size > 0 ? size : 2 * Runtime.getRuntime().availableProcessors();
		this.threadName = new HashMap(this.size);
		this.pools = new ThreadPoolExecutor(size, size,
				0L, TimeUnit.MILLISECONDS,
				new LinkedBlockingQueue<>(1000), (r) -> {
			Thread t = new Thread(this.group, r, this.threadPrefix + "_" + this.threadNumber.incrementAndGet(),
					0L);
			if (t.isDaemon()) {
				t.setDaemon(false);
			}
	...
	public static Thread getThread(long threadId) {
		...
		for (ThreadGroup threadGroup = Thread.currentThread().getThreadGroup(); null != threadGroup; threadGroup = threadGroup.getParent()) {
