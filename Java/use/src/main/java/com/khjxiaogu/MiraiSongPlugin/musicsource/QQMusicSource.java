...
public class QQMusicSource implements MusicSource {
	...
	public String queryRealUrl(String songMid) {
		try {
			...
			JsonObject out = JsonParser.parseString(new String(bs, StandardCharsets.UTF_8)).getAsJsonObject();
			if (out.get("code").getAsInt() != 0) {
				return null;
			}

			return out.get("req_0").getAsJsonObject().get("data").getAsJsonObject()
					.get("sip").getAsJsonArray().get(0).getAsString() + out.get("req_0").getAsJsonObject().get("data").getAsJsonObject().get("midurlinfo")
