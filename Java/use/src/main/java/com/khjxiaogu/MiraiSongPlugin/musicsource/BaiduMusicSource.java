...
public class BaiduMusicSource implements MusicSource {
	...
	public MusicInfo get(String keyword) throws Exception {
		...
		do {
			...
			huc.connect();
			jo = JsonParser.parseString(new String(Utils.readAll(huc.getInputStream()), StandardCharsets.UTF_8)).getAsJsonObject();
		...
		huc.setRequestMethod("GET");
		huc.connect();
		JsonObject allinfo = JsonParser.parseString(new String(Utils.readAll(huc.getInputStream()), StandardCharsets.UTF_8))
				.getAsJsonObject();
