		...
		try {
			if (i < availenc.length)
				for (; i < availenc.length; i++) {
					...
					out = JsonParser.parseString(new String(bs, StandardCharsets.UTF_8)).getAsJsonObject().get("req_0")
							.getAsJsonObject().get("data").getAsJsonObject().get("midurlinfo").getAsJsonArray().get(0)
							.getAsJsonObject().get("purl").getAsString();
			...
			return "http://ws.stream.qqmusic.qq.com/" + out;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
