...
import swing.aitcs.Mode.Node;
import swing.aitcs.Mode.Snake;
...
public class SnakeAi {
	...
	public int play1(Snake s, Node f) {
		...
		Set<String> vis = new HashSet<>();// 记录访问过的节点
		Map<String, String> path = new HashMap<>();//记录访问的路径,后来用A*算法在Node添加了father节点，这个可以去掉了
		Stack<String> stack = new Stack<>();//蛇去吃的路径
	...
	public int play2(Snake snake, Node f) {
		...
		virSnake.setS( new ArrayList<>(snake.getS()));
		virSnake.setMap(new HashSet<>(snake.getMap()));
...
//						System.out.println("ok");
	...
	public int Asearch(Snake s, Node f) {
		...
		Stack<Node> stack = new Stack<>();//蛇去吃的路径
