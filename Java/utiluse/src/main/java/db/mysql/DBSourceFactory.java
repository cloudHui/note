...
public class DBSourceFactory {
	...
	private final Map<String, DBSourceFactory.EnvNodes> envNodesMap = new HashMap<>();
	...
	public SqlSessionFactory getSqlSessionFactory(String configName, String env) {
		DBSourceFactory.EnvNodes envNodes = this.envNodesMap.get(configName);
		if (null == envNodes) {
			this.envNodesMap.putIfAbsent(configName, new DBSourceFactory.EnvNodes(configName));
