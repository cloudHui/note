...
public class ConnectHandler<T extends ConnectHandler, M> extends ChannelInboundHandlerAdapter implements Sender<T, M> {
	...
	private static final ConnectHandler.ConnectManager connectManager;
	...
	private ConnectHandler.CompleterGroup completerGroup;
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		this.completerGroup = new ConnectHandler.CompleterGroup(this.channel.eventLoop());
	...
	public void channelRead(ChannelHandlerContext ctx, Object o) throws Exception {
		Message innerMsg;
		Handler handler;
		ConnectHandler.Completer completer;
	...
	public CompletableFuture sendMessage(int msgId, Message msg, Map<Long, String> attachments, long timeout) {
		...
		ConnectHandler.Completer completer = new ConnectHandler.Completer(timeout);
	...
	static {
		connectManager = ConnectHandler.ConnectManager.INSTANCE;
	}

	private static class CompleterGroup implements Runnable, Comparable<ConnectHandler.CompleterGroup> {
		private static final Throwable TIMEOUT = new RuntimeException("timeout");
		private Map<Long, ConnectHandler.Completer> completerMap = new ConcurrentHashMap(128);
		...
		public int compareTo(ConnectHandler.CompleterGroup o) {
		...
		public void addCompleter(long sequence, ConnectHandler.Completer completer) {
			this.completerMap.put(sequence, completer);
		}

		public ConnectHandler.Completer popCompleter(long sequence) {
			return (ConnectHandler.Completer) this.completerMap.remove(sequence);
		}
		...
		public void destroy() {
			...
			ConnectHandler.Completer completer = null;
			...
			while (!this.completerMap.isEmpty()) {
				...
				while (var4.hasNext()) {
					...
					completer = (ConnectHandler.Completer) this.completerMap.remove(id);
					if (null != completer) {
						completer.ex = ex;
	...
	private static class ConnectManager {
		private static final ConnectHandler.ConnectManager INSTANCE = new ConnectHandler.ConnectManager();
