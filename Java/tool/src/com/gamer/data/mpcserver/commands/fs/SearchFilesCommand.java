...
public class SearchFilesCommand implements CommandHandler {
    ...
    private void appendSearchHeader(StringBuilder sb, int maxOutputChars, long elapsedMs, SearchState state) {
        ...
        h.append("SEARCH_MS=").append(elapsedMs)
            .append(" nodesVisited=").append(state.nodesVisited)
            .append(" maxResults=").append(state.maxResults)
            .append(" maxDepth=").append(state.maxDepth)
            .append(" maxNodes=").append(state.maxNodes)
            .append(" depthLimited=").append(state.depthLimited)
            .append(" resultLimited=").append(state.resultLimited)
            .append(" nodeLimited=").append(state.nodeLimited);
        appendWithLimit(sb, h.toString(), maxOutputChars);
    }
