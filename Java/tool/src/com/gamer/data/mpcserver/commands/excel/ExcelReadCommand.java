...
public class ExcelReadCommand implements CommandHandler {
    ...
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        if (stream) {
            ...
            sb.append("READ_MODE=STREAMING_XLSX\n");
            sb.append("FILE=").append(file.getAbsolutePath()).append('\n');
            sb.append("SHEET=").append(effName).append('\n');
            sb.append("RANGE_ROWS=").append(b.startRow).append(':').append(b.endRow).append(" COLS=").append(b.startCol)
                .append(':').append(b.endCol).append('\n');
    ...
    private static Sheet resolveSheetDom(Workbook wb, String sheetName, Integer sheetIndex) {
        ...
        if (sheetIndex != null) {
            if (sheetIndex.intValue() < 0 || sheetIndex.intValue() >= wb.getNumberOfSheets()) {
                throw new IllegalArgumentException("sheetIndex越界: " + sheetIndex);
            }
            return wb.getSheetAt(sheetIndex.intValue());
        }
    ...
    private static String resolveSheetNameForXlsx(File file, String sheetName, Integer sheetIndex) throws Exception {
        ...
        if (sheetIndex != null) {
            if (sheetIndex.intValue() < 0 || sheetIndex.intValue() >= names.size()) {
                throw new IllegalArgumentException("sheetIndex越界: " + sheetIndex);
            }
            return names.get(sheetIndex.intValue());
        }
        return names.get(0);
    }
