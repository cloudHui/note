 ...
 *
 * <p>说明：POI 的 SAX 路径仍会为 {@link ReadOnlySharedStringsTable} 解析 sharedStrings.xml；
 * 对大表主要收益是<strong>不按行构建整张 Sheet 的 DOM</strong>，并可早停于 endRow 之后。</p>
 *
 * <p>约束：与项目其它 MCP 命令一致，不使用 lambda/stream/try-with-resources。</p>
 */ 
...
public final class ExcelXlsxEventHelper {
    ...
    public static boolean shouldStreamRead(File file, int startRow, int startCol, int endRow, int endCol) {
        if (!isXlsxFile(file) || file == null || !file.isFile()) {
            return false;
        }
        ...
        return file.length() >= STREAM_FILE_BYTES_THRESHOLD
            || spanRows >= STREAM_MIN_ROW_SPAN
            || spanCols >= STREAM_MIN_COL_SPAN
            || estCells >= STREAM_MIN_CELL_ESTIMATE;
    }
     ...
     *
     * @param showFormula true 时尽量输出公式文本（由 POI 事件层 formulasNotResults 控制）
     */
    public static String readRangeAsTsvBody(File xlsxFile, String sheetName, int startRow, int startCol,
        int endRow, int endCol, boolean showFormula) throws Exception {
        ...
        try {
            ...
            SheetContentsHandler sch = handler;
            XSSFSheetXMLHandler sheetHandler =
                new XSSFSheetXMLHandler(styles, null, sst, sch, dataFormatter, showFormula);
    ...
    private static final class TsvSheetContentsHandler implements SheetContentsHandler {
        ...
        private final TreeMap<Integer, String> colValues = new TreeMap<Integer, String>();
        ...
        public void cell(String cellReference, String formattedValue, XSSFComment comment) {
            ...
            colValues.put(Integer.valueOf(c), v);
        }
        ...
        private void flushCurrentRow(int rowNum) {
            ...
            for (int c = startCol; c <= endCol; c++) {
                sb.append('\t');
                String v = colValues.get(Integer.valueOf(c));
                if (v != null) {
                    sb.append(v);
                }
            }
